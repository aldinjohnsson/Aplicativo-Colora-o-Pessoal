...
export function PhotoGallery({ photos, onDownloadAll, loading = false }: PhotoGalleryProps) {
  ...
  const handleDownload = async (photo: Photo) => {
    try {
      ...
      // Se tem URL (Supabase), fazer download via fetch
      if (photo.url) {
      ...
      throw new Error('Foto não disponível para download')
    } catch (error) {
  ...
  const getPhotoUrl = (photo: Photo): string => {
    try {
      // Se tem URL (Supabase), usar ela
      if (photo.url && !photo.url.startsWith('blob:')) {
        return photo.url
      }
      
      // Se tem blob, criar URL do blob
      if (photo.blob) {
            ...
            {photos.map((photo, index) => {
              ...
              return (
                <div
                  ...
                  onClick={() => !hasError && !isLoading && openFullscreen(index)}
                >
                  {!hasError ? (
                    <>
                      {isLoading && (
